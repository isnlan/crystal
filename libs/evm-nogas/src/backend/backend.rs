...
use crate::backend::key_mapping::StorageDoubleMap;
use crate::backend::Vicinity;
...
impl<'vicinity, T: KeyValueDB> CrystalBackend<'vicinity, T> {
    ...
    fn gen_slot_key_prefix(address: H160) -> Vec<u8> {
        Self::address_key(address).storage_double_map_key_prefix(STORAGE_SLOT_KEY)
    }

    // evm:address -> account
    fn gen_account_key(address: H160) -> Vec<u8> {
    ...
    fn get_account(&self, address: H160) -> Result<Option<Account>> {
        let v = self.state.get(0, &Self::gen_account_key(address))?;
        match v {
            Some(data) => Ok(rlp::decode(&data).map_err(|e|anyhow!("rlp decode error: {:}", e))?),
            None => Ok(None),
        }
    }
            ...
            match apply {
                Apply::Modify {
                    ...
                    if let Some(code) = code {
                        ...
                        tx_sub.put(0, Self::gen_code_key(address, code_hash).as_ref(), &code)
                    }

                    if reset_storage {
                        tx_sub.delete_prefix(0, &Self::gen_slot_key_prefix(address))
                    }

                    let storage_prefix = Self::gen_slot_key_prefix(address);
                            ...
                            || self.state.has_key(0, &Self::gen_code_key_perfix(address))?)
                    {
                        ...
                        tx.delete_prefix(0, &Self::gen_account_key(address))
                    } else {
                        tx_sub.put(0, &Self::gen_account_key(address), &rlp::encode(&account));

                        tx.ops.append(&mut tx_sub.ops);
                    }
                }
                Apply::Delete { address } => tx.delete_prefix(0, &Self::gen_account_key(address)),
            }
        }
